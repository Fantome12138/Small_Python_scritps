...
if __name__ == "__main__":
    import cv2
    path = "/Users/fantome/Library/CloudStorage/OneDrive-个人/Git/QR_img/02.jpg"
    img = cv2.imread(path)
    x = img.transpose(2, 0, 1)
    x = x[0:1]
    kernel = np.asarray([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=np.float16).reshape(-1, 1, 3, 3)
    out = conv2d(x, kernel, (1, 2))
    out = np.abs(out)
