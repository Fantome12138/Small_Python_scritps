...
class Detector():
    def __init__(self):
        self.objectList = []
        self.weights = '/home/yolov5-6.0/best_module/cabinet_best.pt'  # 自己的预训练模型
        self.dnn = False
        self.data = "/home/autoLabel/sources/data.yaml"  # 选择配置文件(一般为.yaml)
        self.device = select_device() # 如果使用GPU，则不填写；如果使用cpu，则填写：device='cpu'
        # self.half = self.device.type != 'cpu'  # 半精度化
        self.half = False
        self.predefined_classes = []
        self.imgdir = "/home/imgs/all_img/"  # 你需要标注的图片文件夹
        self.outdir = "/home/imgs/xml/"  # 你需要保存的xml文件夹
        self.detect_class = "/home/autoLabel/sources/detector_classes.txt"  # 你需要自动标注的检测目标类型
