...
def getGtAreaAndRatio(label_dir):
    ...
    for txt in txts:  # 遍历每一个txt文件
        ...
        for line in lines:  # 遍历当前txt文件中每一行的数据
            ...
            ratio = round(float(coor_list[2]) / float(coor_list[3]), 2)  # 计算出当前txt文件中每一个gt的 w/h 长宽比
...
def getAllSMLGtNum(data_dict, class_num):
    """
    数据集所有类别小、中、大GT分布情况
    """
    S, M, L = 0, 0, 0
...
def plotAllSML(SML):
    ...
    plt.title('训练集小、中、大GT分布情况(640x640)', fontsize=16)
    plt.show()
    # 保存到本地
    plt.savefig("/home/test/plot.jpg")
...
if __name__ == '__main__':
    labeldir = '/home/CabinetData/all_labels/'
    data_dict = getGtAreaAndRatio(labeldir)
    # 1、数据集所有类别小、中、大GT分布情况
    class_num = 1  # 数据集内目标类别
    SML = getAllSMLGtNum(data_dict, class_num)
    plotAllSML(SML)
